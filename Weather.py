class CurrentWeather:
    ...
    def get_wind_speed_without_dir(self):
        wind_dir_str_len = 2
        if self.weather_data[self.city][2][-2:-1] == ' ':
            wind_dir_str_len = 1
        speed_str_length = len(
            self.weather_data[self.city][2]) - wind_dir_str_len
        return self.weather_data[self.city][2][0:speed_str_length]
...
if __name__ == "__main__":
    ...
    print("speed without direction using the class function",
          weather.get_wind_speed_without_dir())